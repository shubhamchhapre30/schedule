<?php 
    ...
    $datetimezone = date('Y-m-d');
    $datetimezone = date($site_setting_date,  strtotime($datetimezone));
?>

<link href="<?php echo $theme_url;?>/assets/css/components.min.css?Ver=<?php echo VERSION;?>" rel="stylesheet" type="text/css" />
<link href="<?php echo $theme_url;?>/assets/css/capacityboard.css?Ver=<?php echo VERSION;?>" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="<?php echo $theme_url;?>/assets/plugins/sparkline.min.js?Ver=<?php echo VERSION;?>"></script>
<script type="text/javascript" src="<?php echo $theme_url;?>/assets/scripts/Chart.min.js?Ver=<?php echo VERSION;?>"></script>
...
<script>
    ...
    function show_capacity_calendar(){
        $(function () {
            $('#daterange').daterangepicker({
                ...
                endDate: '<?php echo date($site_setting_date,  strtotime($end_date));?>',
                ranges: {
                    'Today': [moment(), moment()],
            ...
            function(start, end, label) {
                ...
                $.ajax({
                    ...
                    data: {
                        mydate: start.format('YYYY-MM-DD')+"#"+end.format('YYYY-MM-DD'),
                        graph_type:t,
    ...
    function chnage_capacity_mode(){
        $(function () {
            $("#change_mode").bootstrapToggle(),
            $("#change_mode").bootstrapToggle().on("change", function() {
                var chk = $(this).prop('checked');
                var t=$(this).prop('checked')?'graph':'number';
                $("#dvLoading").fadeIn("slow");
                var start_date = $("#start_date_cap").val();
                var end_date = $("#end_date_cap").val();
                $.ajax({
                    type: "POST",
                    url: SIDE_URL + "user/ajax_capacity_dashboard",
                    data: {
                        mydate: start_date+'#'+end_date,
                        graph_type:t,
                        user_filter:$('#filter_value').val(),
                        select_user:$('#select_user').val()
                    },
                    success: function(a) {
                        setCookie("capacity_mode",chk,1);
                        $("#capacity_board").html(a);
                        $("#dvLoading").fadeOut("slow");
                    }
                });
                ...
                <div class="mycaledar-table row board-css">
                    <div class="cal-currentdate col-md-12 btn_show">
                        <div class="col-md-2" >
                            ...
                            <label  class="control-label padding-top-8" style="margin-top: 0px !important;color:#fff"><b>View :</b></label>
                            <input type="checkbox" id="user_filter" name="user_filter" data-toggle="toggle" data-style="android" checked data-width="70" data-on="Me" data-off="Team"  onchange="user_filter()" >
                            <input type="hidden" name="filter_value" id="filter_value" value="me" />
                        </div>
                        <div class="col-md-6" >
                            <div id="filter_list" style="padding-top: 4px;display: none;">
                                <select class="large m-wrap radius-b select_user_menu" name="select_user" id="select_user" tabindex="1" onchange="on_filter();">
                                    ...
                                    {?>
                                        <option value="reported_user" >My Team - Direct Reports only</option>
                                        <option value="all">My Team - All</option><?php
                                    }?>
                                        <?php if($this->session->userdata('is_manager')=='1' && $this->session->userdata('is_administrator') == '0')
                                    {?>
                                        <option value="reported_user" >My Team - Direct Reports only</option> <?php
                                    }?>
                                    <?php foreach ($users as $user){?>

                                    <option value="<?php echo $user->user_id;?>" <?php if($user->user_id == $this->session->userdata('user_id'))echo 'selected';?>><?php if($user->user_id == $this->session->userdata('user_id'))echo 'Me';else echo $user->first_name;?></option><?php }?>
                                </select>
                            </div>
                        </div>
			<div class="col-md-4"  style="padding-left:60px;">
                            <div id="daterange" value="<?php echo $datetimezone;?> - <?php echo $end_date;?>">
                ...
                <div class="row row_box">
                        ...
                        <?php $weekly_tasks = get_calender_weekly_tasks($start_date,$end_date,'all','all',$this->session->userdata('user_id'),'0000-00-00','0','1',$completed,'completed');
                        $total_tasks = 0;
                        $on_time = 0;
                        $late = 0;
                        $actul_work_time = 0;
                         ...
                         foreach ($date_range as $date){
                             ...
                             $daydate = date("M d", strtotime($date));
                                                ...
                                                if(isset($weekly_tasks[$date]) && $weekly_tasks[$date] != ''){
                                                    ...
                                                    $week = date("d M", $tm - (86400 * $w) ).' to '.date("d M", $tm + 86400 * (6 - $w) ) ;
                                                ...
                                                $hour_array[$i]['date'] = $date;
                                                $hour_array[$i]['value'] = round($billable_hours/60,2);
                                                $hour_array[$i]['view'] = round($non_billable_hours/60,2);
                                                $hour_array[$i]['unit'] = 'h';
                                                $cost_array[$i]['date'] = $date;
                                                $cost_array[$i]['value'] = round($billable_cost);
                                                $cost_array[$i]['view'] = round($non_billable_cost);
                                                $cost_array[$i]['unit'] = $this->session->userdata('currency');;
                                                $demo_array[$i]['date'] = $date;
                                                $demo_array[$i]['value'] = rand(1, 10);
                                                $demo_array[$i]['view'] = rand(1, 10);
                                                $demo_array[$i]['unit'] = 'h';
                                                $i++;
                    ...
                    <?php
                       if($total_tasks > 0)
                       {$width = 100;$active_users = 1;}
                       else
                       {$width = 0;$active_users = 0;}
                           ?>
                    <?php $prg_width = 0; ?>
                        <div class="col-md-3 ">
                            <div class="common_widget dashboard-stat2">
                                ...
                                <div class="display">
                                    <div class="number">
                                        ...
                                        <h3 class="font-blue-sharp" style="padding-top: 10px;">
                                            <span data-counter="counterup" data-value="1899"><?php echo $active_users;?>/1</span>
                                        </h3>
                                    </div>
                                    <div class="icon">
                                        <i class="icon-user"></i>
                            ...
                            <div class="portlet light ">
                                <div class="portlet-title">
                                    ...
                                    <div class="actions" style="padding-top: 15px !important;">
                                        <input type="hidden" name="start_date_cap" id="start_date_cap" value="<?php echo $start_date; ?>"/>
                                        <input type="hidden" name="end_date_cap" id="end_date_cap" value="<?php echo $end_date; ?>"/>
                                        <input type="checkbox" id="change_mode" data-toggle="toggle" data-style="android" data-width="120" data-on="<i class='fa  fa-bar-chart'></i> Graphical" data-off="<i class='fa fa-line-chart'></i> Numerical"  <?php  echo (isset($_COOKIE['capacity_mode']) && $_COOKIE['capacity_mode']=='false') ? "" : "checked";?> >
                                    </div>
                                </div>
                                        ...
                                        <table class="table table-striped  table-condensed table-bordered " style=" width: auto !important;border:2px solid #e7ecf1 !important;"  >
                                                    <thead class="flip-content">
                                                      <tr>
                                                            ...
                                                            <?php foreach($date_range as $date) {
                                                                ...
                                                                <th <?php echo $tdstyle; ?> ><span <?php if($date == $end_date){ echo 'style="float:left"'; } ?>><?php echo date('D d',  strtotime($date)); ?><span style="display:block"><?php echo date('M',  strtotime($date)); ?></span></span><?php if($date == $end_date){?><span style="float: right;padding-top: 9px;"><a href="javascript:void(0)" onclick="change_view_capacity('<?php echo $end_date."#next"; ?>');"> <i class="glyphicon glyphicon-chevron-right"> </i> </a></span> <?php } ?></th>           
                                                            <?php } ?>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <?php 
                                                            $user_total_capacity = 0;$total_estimate_time = 0;
                                                            foreach($date_range as $date){
                                                                    $estimate_hours = 0;
                                                                    $estimate_minutes= 0;
                                                                    $total_estimate = 0;
                            ...
                            <div class="portlet light ">
                                <div class="portlet-title">
                                    ...
                                    <div class="actions" style="padding-top:12px">
                                        <input type="checkbox" <?php if($this->session->userdata('pricing_module_status')=='1'){echo '';}else{ echo 'disabled';}?> id="graph_mode" data-toggle="toggle" data-style="android" data-width="75" onchange="change_graph_data();" data-on="Hours" data-off="$" <?php echo (isset($_COOKIE['billable_graph_type']) && $_COOKIE['billable_graph_type'] == 1) ?'':'checked';?> > 
                                    </div>
                                </div>
                                <div class="portlet-body">
                            ...
                            <div class="portlet light ">
                                ...
                                <div class="portlet-body" >
                                    <div class="customtable form-horizontal">
                                        <table class="table table-striped table-hover table-condensed flip-content <?php if($this->session->userdata('pricing_module_status')=='0' ||$this->session->userdata('customer_module_activation')=='0') {echo 'disable_customer';} ?>" id="customerTable" style="<?php if($this->session->userdata('customer_module_activation')=='0'){ echo 'background-color:grey;';} ?>">
                                            <thead class="flip-content">
                                                <tr>
                                                    <th>Customer Name</th>
                                                    <th>Revenue</th>
                                        ...
                                        <div style="color: white;font-size: 18px;position: absolute;top: 50%;left: 10%;right: 10%;text-align: center;<?php if($this->session->userdata('customer_module_activation')=='1' && $this->session->userdata('pricing_module_status')=='1'){echo 'display:none';}?>">To access this dashboard, you must activate the customer and pricing modules.</div>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript" src="<?php echo $theme_url;?>/assets/plugins/amcharts/amcharts.js?Ver=<?php echo VERSION;?>"></script>
<script type="text/javascript" src="<?php echo $theme_url;?>/assets/plugins/amcharts/serial.js?Ver=<?php echo VERSION;?>"></script>
<script type="text/javascript" src="<?php echo $theme_url;?>/assets/plugins/amcharts/gauge.js?Ver=<?php echo VERSION;?>"></script>
<script type="text/javascript" src="<?php echo $theme_url;?>/assets/plugins/amcharts/themes/light.js?Ver=<?php echo VERSION;?>"></script>
<?php 
$utilization = 0;
...
?>
<script>
    var graph_type = getCookie('billable_graph_type')?getCookie('billable_graph_type'):0;
...
if(total_tasks == 0){
    ...
    var ctx4 = document.getElementById("doughnutData").getContext("2d");
    var chart = new Chart(ctx4, {type: 'doughnut', data: doughnutData, options:doughnutOptions});
...
function user_filter(){
        var t=$("#user_filter").prop('checked')?'me':'team';
        if(t == 'team'){
            $('#filter_list').show();
            $('#filter_value').val(t);
        }else{
...
function randomValue() {
  var value = Math.round( utilization * 100 );
  if ( chart ) {
    if ( chart.arrows ) {
      if ( chart.arrows[ 0 ] ) {
...
var cost_data = <?php echo json_encode($cost_array);?>;
var hour_data = <?php echo json_encode($hour_array);?>;
function change_graph_data(){
    if(graph_type == 0){
        chart1.dataProvider = cost_data;graph_type = 1;
        setCookie("billable_graph_type",graph_type,1);
    }else{
        chart1.dataProvider = hour_data;graph_type = 0;
        setCookie("billable_graph_type",graph_type,1);
    }
    chart1.validateData();
}
...
var date_cat_graph = <?php echo json_encode($graph_cat_data);?>;
var month_cat_graph = <?php echo json_encode($month_cat_data);?>;
var week_cat_graph = <?php echo json_encode($week_cat_data);?>;
var current_cat_graph = <?php echo isset($_COOKIE['cat_graph_type'])?($_COOKIE['cat_graph_type']=='date'?json_encode($graph_cat_data):($_COOKIE['cat_graph_type']=='week'?json_encode($week_cat_data):($_COOKIE['cat_graph_type']=='month'?json_encode($month_cat_data):json_encode($graph_cat_data)))):json_encode($graph_cat_data);?>;
...
function change_cat_graph(cat_graph){
    setCookie("cat_graph_type",cat_graph,1);
    if(cat_graph == 'date'){
        chart2.dataProvider = date_cat_graph;
    }else if(cat_graph == 'month'){
        chart2.dataProvider = month_cat_graph;
    }else if(cat_graph == 'week'){
        chart2.dataProvider = week_cat_graph;
    }
    chart2.validateData();
}
...
<style>
    <?php if($this->session->userdata('customer_module_activation')=='0'){?>
    .customtable .table-striped.disable_customer tbody > tr:nth-child(2n+1) > td, 
    .customtable .table-striped.disable_customer tbody > tr:nth-child(2n+1) > th{
        background-color: grey !important;
        border-top: 0 !important;
    }
    .table.disable_customer>tbody>tr>td, 
    .table.disable_customer>tbody>tr>th{
        border-top: 0 !important;
    }
