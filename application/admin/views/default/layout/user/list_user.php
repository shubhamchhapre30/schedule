<script type="text/javascript" language="javascript">
	...
	function delete_rec(id,redirectpage,option,keyword,limit,offset)
	{
		...
		if(ans)
		{
			location.href = "<?php echo site_url("user/delete"); ?>/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
...
{
	var ans = confirm("Are you sure, you want to send reset password E-mail for this User?");
	if(ans)
	{
		location.href = "<?php echo site_url("user/reset_password_user"); ?>/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
	}else{
		return false;
	}
}
...
<div class="page-content">
	...
	<div class="container-fluid admin-list">
		...
		<div class="row">
			<div class="col-md-12">				
				<h3 class="page-title"> User List <noscript> DIsable </noscript></h3>				
			</div>
		</div>
		<!-- END PAGE HEADER-->
		<!-- BEGIN PAGE CONTENT-->
			...
			<div class="col-md-12">
				...
				<div class="portlet box green">
					...
					<div class="input-append">
						...
						<div class="btn-group">
						<?php echo anchor('user/list_user','<i class="fa fa-refresh"></i>','class="btn green icn-only"');?>
						</div>
				    </div>
				   </form>
				   </div>
				   <?php //print_r($adminRights);?>
					...
					<?php echo ((isset($adminRights->user) && $adminRights->user->add==1) || checkSuperAdmin())?anchor('user/add','Add New <i class="fa fa-plus"></i>','class="btn green"'):''?>
					</div>
					<?php if((isset($adminRights->user) && ($adminRights->user->update==1 || $adminRights->user->delete==1)) || checkSuperAdmin()){ ?>
					...
					<div class="btn-group">
										...
										<ul class="dropdown-menu pull-right">
											<?php if((isset($adminRights->user->update) && $adminRights->user->update==1) || checkSuperAdmin()){?><li><a href="javascript:void(0)" onclick="setaction('chk[]','active', 'Are you sure, you want to activate selected record(s)?', 'action_user');"><i class="fa fa-thumbs-up"></i> Active</a></li>
											...
											<?php } if((isset($adminRights->user->delete) && $adminRights->user->delete==1) || checkSuperAdmin()){ ?><li><a href="javascript:void(0)" onclick="setaction('chk[]','delete', 'Are you sure, you want to delete selected record(s)?', 'action_user');"><i class="fa fa-trash"></i> Delete</a></li>
												<?php } ?>
											<li class="divider"></li>
										</ul>
									</div>
					...
					echo form_open('user/action',$attributes);?>
				...
				foreach($result as $row) {
					...
					<td align="center"> <?php if($row->is_administrator !='1'){?><input type="checkbox"  class="checkboxes" name="chk[]" id="titleCheck<?php echo  $row->user_id;?>" value="<?php echo $row->user_id;?>"><?php } ?></td>
					...
					if($row->profile_image!="" && $this->s3->getObjectInfo($bucket,'upload/user/'.$row->profile_image)) { $img= $row->profile_image; } else { $img='no_image.jpg';}?>
					<td align="center"><img src="<?php echo $s3_display_url.'upload/user/'.$img; ?>" style="width:50px;height:50px;" /></td>
					<td><?php echo $row->first_name; ?></td>
					<td><?php echo $row->last_name; ?></td>
					
					<!--<td><?php if($row->user_type=="business") echo "Customer working for a Naturalia company"; else echo "Particular"; ?></td>-->
					<td><?php echo $row->email; ?></td>
					<td><?php  echo $row->company_name; ?></td>	
					<td><?php if($row->is_administrator=="1") echo "Yes"; else echo "No"; ?></td>
					<td><?php if($row->is_owner=="1") echo "Yes"; else echo "No"; ?></td>
					...
					<?php
					if((isset($adminRights->user) && ($adminRights->user->update==1 || $adminRights->user->delete==1)) || checkSuperAdmin()){ ?>
					<td align="center">
					<p>	
						<?php 
					 if((isset($adminRights->user->update) && $adminRights->user->update==1) || checkSuperAdmin()){?>				
						<a href="javascript:void(0);" onClick="reset_password('<?php echo $row->user_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Send reset password email" class="btn purple icn-only mini"><i class="fa fa-envelope-o m-icon-white"></i></a>
							
					<?php 
					 }
					 if((isset($adminRights->user->update) && $adminRights->user->update==1) || checkSuperAdmin()){
						echo anchor('user/edit/'.$row->user_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="fa fa-edit m-icon-white"></i>','class="btn blue icn-only mini"   title="Edit user"');
					} ?>
					<?php if($row->is_administrator !='1'){
						
					 if((isset($adminRights->user->delete) && $adminRights->user->delete==1) || checkSuperAdmin()){ ?>
					<a href="javascript:void(0);" onClick="delete_rec('<?php echo $row->user_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete" class="btn red icn-only mini"><i class="fa fa-remove fa fa-white"></i></a>
					<?php } } ?>
					</p></td><?php } ?>
					</tr>
					<?php	} }else{?>
	...
	$(document).ready(function() {
		...
		if($msg!=''){
	     ...
	     if($msg == "insert"){ $error = ADD_NEW_RECORD;}
			...
			if($msg == "no_delete"){ $error = NO_DELETE_RECORD;}
