...
/*
    $('#frm_City').on('submit', function(event) {
        var $form = $(this);
        var $target = $($form.attr('data-target'));
 		var limit=$('#limit').val();
...
function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo site_url('City/listCity');?>';
		}
...
function getData(limit,offset)
{
	...
	var url='<?php echo site_url('City') ?>/'+redirect_page+'/'+limit+'/'+sort_on+'/'+sort_type+'/'+offset;
	}else{
		var url='<?php echo site_url('City/') ?>/'+redirect_page+'/'+limit+'/'+option+'/'+keyword+'/'+sort_on+'/'+sort_type+'/'+offset;
	} 
...
function delete_rec(id,redirectpage,option,keyword,limit,offset)
	{
		var ans = confirm("Are you sure, you want to delete  City?");
		if(ans)
		{
/*			location.href = "<?php //echo base_url(); ?>City/deleteCity/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;*/
			$.ajax({
			url : "<?php echo site_url('City/deleteCity') ?>/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+sort_on+'/'+sort_type+'/'+offset,
			cache: false,
...
function getlimit(limit)
	{
		...
		else
		{
			...
			var redirect_page=$('#redirect_page').val();
			if(redirect_page=='listCity'){ 
			var url='<?php echo site_url('City') ?>/'+redirect_page+'/'+limit+'/'+sort_on+'/'+sort_type;
			}else{
				var url='<?php echo site_url('City/') ?>/'+redirect_page+'/'+limit+'/'+option+'/'+keyword+'/'+sort_on+'/'+sort_type;
			} 
		...
		<div class="row-fluid">
			<div class="span12">
				...
				<div class="portlet box green">
					...
					echo form_open('City/searchListCity/'.$limit,$attributes);?>
					<div class="pull-left">
						...
						<div style="margin-right:5px;">
                            ...
                            <option value="city_name" <?php if($option=='city_name'){?> selected="selected"<?php }?>>City Name</option>
                            <option value="country_name" <?php if($option=='country_name'){?> selected="selected"<?php }?>>Country Name</option>
                            <option value="state_name" <?php if($option=='state_name'){?> selected="selected"<?php }?>>Province</option>  
					...
					<div class="input-append">
						...
						<div class="btn-group">
						<?php echo anchor('City/listCity','<i class="icon-refresh"></i>','class="btn blue icn-only"');?>
						</div>
				    </div>
				   </form>
				   </div>
					...
					<div class="btn-group">
					<?php echo ((isset($adminRights->Globalization) && $adminRights->Globalization->add==1) || checkSuperAdmin())?anchor('City/addCity','Add New <i class="icon-plus"></i>','class="btn green"'):'';
					?>
					</div>
					...
					<div class="btn-group">
										...
										<ul class="dropdown-menu pull-right">
											<?php if((isset($adminRights->Globalization->update) && $adminRights->Globalization->update==1) || checkSuperAdmin()){?><li><a href="javascript:void(0)" onclick="setaction('chk[]','active', 'Are you sure, you want to activate selected record(s)?', 'frm_City');"><i class="icon-thumbs-up"></i> Active</a></li>
											<li><a href="javascript:void(0)" onclick="setaction('chk[]','inactive', 'Are you sure, you want to inactivate selected record(s)?', 'frm_City');"><i class="icon-thumbs-down"></i> Inactive</a></li>
											<?php } if((isset($adminRights->Globalization->delete) && $adminRights->Globalization->delete==1) || checkSuperAdmin()){ ?><li><a href="javascript:void(0)" onclick="setaction('chk[]','delete', 'Are you sure, you want to delete selected record(s)?', 'frm_City');"><i class="icon-trash"></i> Delete</a></li>
											<?php } ?>
											<li class="divider"></li>
										</ul>
									</div>
					...
					<!--<div class="btn-group">
						<a href="javascript://" onclick="donwloadCSV()" class="btn black"><i class="icon-download"></i> download</a>
										...
										echo form_open('City/downloadCity',$att) ?>
...
<script>
	...
	$(document).ready(function() {
		...
		<?php if($msg!=''){
	     ...
	     if($msg == "insert"){ $error = ADD_NEW_City;}
            if($msg == "update"){ $error = UPDATE_City;}
            if($msg == "delete"){ $error = DELETE_City;}
			if($msg == "active") {  $error = ACTIVE_City;}
			if($msg == "inactive"){ $error = INACTIVE_City;}
    ?> 
