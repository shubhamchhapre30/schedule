<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete service_type_id?");
		if(ans)
		{
			location.href = "<?php echo site_url("Country/deleteCountry"); ?>/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	{	
		...
		{
			window.location.href='<?php echo site_url("Country/listCountry");?>/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo site_url("Country/searchListCountry/");?>/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	<div class="container-fluid">
		...
		<div class="row-fluid">
			<div class="span12">
				...
				<h3 class="page-title"> Country List <noscript> DIsable </noscript></h3>
			...
			<div class="span12">
				...
				<div class="portlet box green">
					...
					<div class="input-append">
						...
						<div class="btn-group">
						<?php echo anchor('Country/ListCountry','<i class="icon-refresh"></i>','class="btn green icn-only"');?>
						</div>
				    </div>
				   </form>
				   </div>
				   <?php //print_r($adminRights);?>
					...
					<div class="btn-group">
					<?php echo ((isset($adminRights->admin) && $adminRights->admin->add==1) || checkSuperAdmin())?anchor('Country/addCountry','Add New <i class="icon-plus"></i>','class="btn green"'):''?>
					</div>
					...
					<div class="btn-group">
										...
										<ul class="dropdown-menu pull-right">
											<li><a href="javascript:void(0)" onclick="setaction('chk[]','active', 'Are you sure, you want to activate selected record(s)?', 'action_country');"><i class="icon-thumbs-up"></i> Active</a></li>
											<li><a href="javascript:void(0)" onclick="setaction('chk[]','inactive', 'Are you sure, you want to inactivate selected record(s)?', 'action_country');"><i class="icon-thumbs-down"></i> Inactive</a></li>
											<li><a href="javascript:void(0)" onclick="setaction('chk[]','delete', 'Are you sure, you want to delete selected record(s)?', 'action_country');"><i class="icon-trash"></i> Delete</a></li>
				...
				<?php			 
					$attributes = array('name'=>'action_country','id'=>'action_country');
				...
				foreach($result as $row) {
					...
					<?php echo anchor('Country/editCountry/'.$row->country_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="icon-edit m-icon-white"></i>','class="btn blue icn-only mini"   title="Edit admin"'); ?>
